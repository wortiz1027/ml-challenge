...
public class MessageServices implements IMessageServices {
    ...
    public CompletableFuture<Response> getReport(String id) {
        ...
        try {
            ...
            if (!messages.isPresent()) throw new MessageNullException(String.format("Error decoding message with code: [%s]", id));
            ...
            e.printStackTrace();
            throw new ServerException(String.format("There is an error getting message... [%s]", e.getMessage()));
        }

    }

    private List<String[]> buildMessage(Optional<List<Message>> messages) {
        return null;
    }

    private float[] buildDistance(Optional<List<Message>> data) {
        int index = 0;
